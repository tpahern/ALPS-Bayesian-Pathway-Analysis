...
fitalps <- function(iter,curtree,initipsi,normpotts,prioronly) {
	...
	for (i in 1:howlong) {
		...
		write.tree(proposedtree,file=paste0(wd,"/results/testing-sampler-proposed-trees.txt"),append=append,tree.names=i)	
		...
		if (!normpotts) {
			curtree <- updatepsi(curtree,i,usedata)
		}
...
post <- function(tree,usedata) {
	...
	n <- ncol(dos)
	gamma <- 0.5 # for extended BIC, set to 0.5 or 1 for applications with many parameters
...
newtree <- function(tree) {
	...
	# Args:
 	#   tree: the current tree
  	...
  	# In a future version, consider 1-input trees (makes theta=c(0,1) or c(1,0) cases less likely?)
...
alpstreeph <- function(pars, x, xn, y) {
	# This is the likelihood function for a ALPS tree, set up for the ph model
	# Args:
  	#   pars: The model parameters
 	#   x: the tree
 	#	xn: design matrix of observed (tips) and predicted values for the tree
	...
	#*********
	# need to modify so beta1 is the net effect
  	theta <- matrix(pars[1:length(pars)-1],ncol=2,byrow=T)
  	rownames(theta) <- (ntips+1):(Nnode(x)+ntips)  # labels the rows to match the tree
	...
	# WARNING: Assumes time is sorted!  Consider error check in later version.
	a <- pred %*% beta1
	b <- log(rev(cumsum(rev(exp(a)))))
  -sum((a - b)[status.var==1])
...
optimtree <- function(tree) {
  ...
  #  beta1 <- NA #pathway effect
    ...
    pars <- c(theta,0)
  	...
  	lower <- array(-Inf,dim=(length(theta)+1))
  	lower[1:length(theta)] <- 0;
  	
	# set upper constraints
	upper <- array(10,dim=length(theta))
	upper <- c(upper,Inf) 
   	...
   	op <- optim(
      	...
      	x = tree,
      	xn = xn,
      	y = status.var) #,control=list(trace=6));
	...
	tree$negloglik = NA;
  	...
  	if (op$convergence == 0) {
  		tree$beta <- op$par[length(op$par)];
  		tree$theta <- matrix(op$par[1:(length(op$par)-1)],ncol=2,byrow=T)
...
updatepsi <- function(tree,i,usedata) {
