...
# set indicator for 1st time and zero for all the others
tiemask <- as.numeric(!duplicated(time.var))
tiemask[tiemask==0] <- NA
tiemaskix <- which(tiemask==0)

cbind(time.var,tiemask)[1:30,]

library(zoo)

rmask <- rep(1,10)
rmask[2:3] <- NA
rnum <- round(runif(10)*10)
c <- cumsum(rnum)
c*rmask
na.locf(c*rmask,fromLast=TRUE)
...
likfun<- function(beta, data) {
 ...
 # b <- log(rev(cumsum(rev(exp(a)))))
  ...
  b2 <- rev(cumsum(rev(exp(a))))
  b2[tiemaskix] <- NA; 
  b2 <- log(na.locf(b2))
 
 # b2 <- log(rev(cumsum(rev(exp(a)))*tiemask))))

#b2 <- with(data, ave(b, time.var, FUN = max))
#b2b <- ave(b, time.var, FUN = max)
